...
import SocialLinkCard from "./SocialLinkCard"; // Import the correct component
...
const socialLinks = [
  ...
  },
  {
    ...
    subText: "@urfavoli",
    icon: Instagram,
    url: "https://www.instagram.com/urfavoli",
    color: "#E4405F",
  ...
  },
  {
    ...
    subText: "@eki zulfar",
    icon: Youtube,
    url: "https://www.youtube.com/@urfavoli",
    color: "#FF0000",
  ...
  },
  {
    ...
    color: "#24292e", // Using a dark gray for GitHub color
  ...
  },
  {
    ...
    icon: ({ className, ...props }: React.SVGProps<SVGSVGElement>) => (
      <svg
        ...
        className={className} // Pass className to allow sizing
        {...props}
      >
        <title>Tiktok</title>
...
const SocialLinks = () => {
  useEffect(() => {
    AOS.init({
      offset: 50, // Slightly reduced offset
  ...
  return (
    <div className="w-full bg-gradient-to-br from-white/10 to-white/5 rounded-2xl p-6 py-8 backdrop-blur-xl relative overflow-hidden">
      {/* Background radial glow */}
      <div className="absolute inset-0 bg-gradient-to-br from-indigo-900/20 to-purple-900/10 rounded-2xl blur-xl animate-pulse-slow"></div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {secondaryLinks.map((link, index) => (
            <SocialLinkCard
              key={link.name}
              {...link}
              aosDelay={200 + index * 100} // Dynamic delays
            />
          ))}
        </div>
      </div>
