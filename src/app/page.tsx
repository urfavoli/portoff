...
const dummyProjects: Project[] = [
  {
    ...
    description: "A full-stack web application built with Next.js, and a PostgreSQL database. Features include user authentication, product listings, shopping cart, and order management.",
    liveDemoLink: "https://wavei.vercel.app",
  },
  {
    id: "2",
    imgSrc: "/pica.png",
    ...
    liveDemoLink: "https://soufianey.vercel.app",
  },
  {
    id: "3",
    imgSrc: "/pico.png",
...
const SocialLinkCard: React.FC<SocialLinkProps> = ({ displayName, subText, icon, url, gradient, aosDelay }) => (
  <a
    ...
    className={`flex items-center gap-4 p-4 rounded-xl text-white transition-all duration-300 hover:scale-[1.02] transform bg-gradient-to-r ${gradient} relative z-10 overflow-hidden shadow-lg`}
    data-aos="fade-up"
...
const CardProject: React.FC<Project & { onClick: () => void }> = ({ imgSrc, title, description, liveDemoLink, onClick }) => (
  <div
    className="bg-white/5 p-4 rounded-xl shadow-lg border border-white/10 flex flex-col items-center cursor-pointer hover:scale-105 transition-transform duration-300 relative overflow-hidden group hover:shadow-2xl"
    data-aos="fade-up"
    data-aos-delay="200"
    onClick={onClick}
    role="button"
  ...
  >
    <div className="absolute inset-0 opacity-0 transition-opacity duration-500 group-hover:opacity-100 pointer-events-none glowing-border animate-spin-slow"></div>
    <div className="relative w-full h-48 mb-4 overflow-hidden rounded-lg">
      <Image
        src={imgSrc}
        alt={title}
        fill
        className="rounded-lg transition-transform duration-500 group-hover:scale-110 object-cover"
      />
    </div>
    ...
    <a 
      ...
      target="_blank" 
      rel="noopener noreferrer" 
      className="mt-auto inline-block px-4 py-2 rounded-full bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-semibold hover:from-purple-700 hover:to-indigo-700 transition-colors"
...
const ProjectCardModal: React.FC<ProjectModalProps> = ({ project, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md p-4">
      <div 
        ref={modalRef}
        className="bg-gray-800 p-8 rounded-lg max-w-xl w-full text-center relative"
      >
        <button 
          onClick={onClose} 
          className="absolute top-4 right-4 text-white hover:text-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 rounded-full p-1"
        ...
        <a 
          ...
          target="_blank" 
          rel="noopener noreferrer" 
          className="inline-block px-6 py-3 rounded-full bg-gradient-to-r from-blue-600 to-purple-600 text-white font-semibold hover:from-blue-700 hover:to-purple-700 transition-colors"
        >
          Visit Website
        </a>
      </div>
...
const Certificate: React.FC<CertificateData> = ({ ImgSrc, title = "Certificate" }) => {
  ...
  return (
    <div 
      className="relative w-[300px] h-[200px] md:w-[400px] md:h-[250px] flex-shrink-0 cursor-pointer overflow-hidden rounded-lg shadow-lg transition-all duration-300 ease-in-out hover:scale-105 group"
      data-aos="fade-up" 
      data-aos-delay="100"
      onClick={handleOpen} 
      role="button" 
    ...
    >
      <div className="absolute inset-0 z-0 opacity-0 transition-opacity duration-500 group-hover:opacity-100 pointer-events-none glowing-border animate-spin-slow"></div>
      ...
      <div className="relative w-full h-full z-10">
        <Image
          ...
          alt={`${title} Thumbnail`}
          fill
          className="transition-transform duration-500 ease-out group-hover:scale-110 object-contain"
        />
        <div className="absolute inset-0 bg-black/30 opacity-0 transition-opacity duration-300 group-hover:opacity-100 flex items-center justify-center">
          <div className="text-white text-center opacity-0 translate-y-2 transition-all duration-400 group-hover:opacity-100 group-hover:translate-y-0">
            <Fullscreen className="w-10 h-10 mb-2 drop-shadow-md" />
            <h6 className="font-semibold drop-shadow-lg text-lg">View Certificate</h6>
          </div>
        </div>
      </div>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md animate-fadeIn">
          <div className="relative w-full h-full p-4 flex items-center justify-center">
            <button 
              onClick={handleClose} 
              className="absolute top-4 right-4 z-10 p-2.5 rounded-full bg-black/60 text-white transition-all duration-300 hover:bg-black/80 hover:scale-110 hover:rotate-90 focus:outline-none focus:ring-2 focus:ring-white"
              aria-label="Close modal"
            >
              <X className="w-7 h-7" />
            </button>
...
const InputField: React.FC<InputFieldProps> = ({ field, label, icon: Icon, formData, handleChange }) => {
  const renderInput = () => {
    const baseClasses = "w-full p-4 rounded-xl bg-gray-800 text-white placeholder-transparent focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all duration-300 peer shadow-sm hover:shadow-md";
  ...
  return (
    <div className="relative w-full group" data-aos="fade-up" data-aos-delay="200">
      ...
      <label
        htmlFor={field}
        className={`absolute left-12 top-4 transform -translate-y-1/2 text-gray-400 text-sm transition-all duration-300 peer-placeholder-shown:top-1/2 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:text-base peer-focus:top-4 peer-focus:-translate-y-1/2 peer-focus:text-indigo-400 peer-focus:text-sm`}
      >
        {label}
      </label>
    </div>
...
const HomePage = () => {
  ...
  return (
    <main className="min-h-screen bg-[#030014] text-white font-sans overflow-x-hidden relative">
      <AudioPlayer src="/your-music-file.mp3" />
      <style jsx global>{`
        body { 
      ...
      <section id="home" className="min-h-screen flex items-center justify-center pt-20 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
        <div className="text-center max-w-4xl mx-auto py-20 z-10" data-aos="fade-up">
          ...
          <p className="mt-6 text-lg md:text-xl lg:text-2xl text-gray-300 max-w-2xl mx-auto">
            I'm Soufiane, a passionate Developer building digital useful apps.
          </p>
          <a
            href="#portfolio"
            className="mt-10 inline-block px-8 py-4 rounded-full bg-gradient-to-r from-purple-600 to-indigo-600 text-white text-lg font-semibold hover:from-purple-700 hover:to-indigo-700 transition-all duration-300 hover:shadow-glow"
          >
            Explore My Work
          </a>
        </div>
          ...
          <div className="flex flex-col lg:flex-row items-center gap-12">
            <div className="lg:w-1/2 text-left space-y-6">
              <p className="text-lg text-gray-300 leading-relaxed">
                Hello! I'm <strong>Soufiane</strong>, a dedicated web developer with a passion for creating dynamic and responsive web applications. My journey into development started with a curiosity for how websites work, evolving into a drive to build intuitive and efficient user experiences.
              </p>
              <p className="text-lg text-gray-300 leading-relaxed">
                I specialize in front-end and back-end development with a strong foundation in React and Next.js, complemented by modern CSS frameworks like Tailwind CSS. I'm always eager to learn new technologies and solve complex problems, turning ideas into reality one line of code at a time.
              </p>
            </div>
      ...
      <section id="portfolio" className="py-20 bg-[#030014] border-t border-white/5 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto text-center" data-aos="fade-up">
          <h2 className="text-4xl md:text-5xl font-bold text-white mb-12">My Projects</h2>
      ...
      <section id="certificates" className="py-20 bg-[#06061a] border-t border-white/5 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto text-center" data-aos="fade-up">
          <h2 className="text-4xl md:text-5xl font-bold text-white mb-12">My Certificates</h2>
          <p className="text-lg text-gray-300 mb-16">
            Proof of my continuous learning and dedication to mastering web technologies.
          </p>
          <div className="certificates-container">
            {[...dummyCertificates, ...dummyCertificates].map((cert, index) => (
              <div key={`${cert.title}-${index}`} className="certificates-item">
                <Certificate ImgSrc={cert.ImgSrc} title={cert.title} />
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-xl mx-auto text-center" data-aos="fade-up">
          ...
          >
            ...
            <button
              ...
              className="w-full py-3 px-6 rounded-lg bg-gradient-to-r from-blue-600 to-purple-600 text-white font-semibold shadow-md hover:from-blue-700 hover:to-purple-700 transition-all duration-300 hover:scale-[1.02] transform disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-indigo-500"
            >
      ...
      {modalOpen && selectedProjectForModal && (
        <ProjectCardModal
